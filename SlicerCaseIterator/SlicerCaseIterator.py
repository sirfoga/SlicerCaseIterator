...
class SlicerCaseIterator(ScriptedLoadableModule):
    ...
    def __init__(self, parent):
        ...
        self.parent.contributors = [
            "Joost van Griethuysen (AVL-NKI), Christian Herz (CHOP)"
        ]
...
class SlicerCaseIteratorWidget(ScriptedLoadableModuleWidget):
    ...
    def setup(self):
        ...
        try:
            self.txtReaderName.text = str(os.getlogin())
        except:
            self.txtReaderName.text = "unknown"

        self.txtReaderName.toolTip = (
            "Name of the current reader; if not empty, this name will be added to the filename "
            "of saved masks"
        )
        ...
        self.sliceFill2DSlider.value = 0.0
        ...
        # Load CSV / Next Case
        #
        self.nextButton = qt.QPushButton("(Save and) Next Case")
        ...
        #
        # auto self.modeComboBox.currentTextChanged.connect(self.onModeSelected)
    ...
    def setupViewSettingsArea(self):
        try:
            ...
            logging.info(
                "Skipping (optional) view settings area: SlicerDevelopmentToolbox was not found which is required "
    ...
    def onReset(self):
        if self.logic is None:
            ...
            try:
                ...
                self.logic = SlicerCaseIteratorLogic(iterator, self.npStart.value)
                self.logic.start()
                self.updateSegmentationProperties()
                self._setGUIstate()
    ...
    def onNext(self):
        ...
        if self.logic.nextCase():
            ...
            dialog.setWindowTitle("All good :)")
            layout = qt.QVBoxLayout(dialog)
            label = qt.QLabel("Fertig!")
            layout.addWidget(label)
            button = qt.QPushButton("OK")
            button.connect("clicked()", dialog.accept)
    ...
    def _unlockGUI(self, unlocked):
        ...
        if unlocked:
            self.nextButton.text = "(Save and) Next Case"
        else:
...
class SlicerCaseIteratorLogic(ScriptedLoadableModuleLogic):
    ...
    def __init__(self, iterator, start):
        ...
        assert self.iterator.caseCount >= start, (
            "No cases to process (%d cases, start %d)"
    ...
    def _loadCase(self):
        ...
        if not self.iterator.loadCase(self.currentIdx):
            print(f"Skipping Case #{self.currentIdx:d}")
            if self.currentIdx >= self.iterator.caseCount:
                self.logger.info("########## All Done! ##########")
                return True
            else:
